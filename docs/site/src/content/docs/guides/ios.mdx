...
- **Simulator:** `xcrun simctl spawn <deviceId> launchctl list` to enumerate running processes
- **Physical device:** `pgrep -f <bundleId>` to find the process by bundle identifier
...
A single snapshot of the process's resident set size (rsize) is captured to measure memory consumption in megabytes.
...
|--------|-----------|-----------------|
| Process discovery | `xcrun simctl spawn` | `pgrep -f` |
...
| Setup | No additional setup | Developer Mode required |
