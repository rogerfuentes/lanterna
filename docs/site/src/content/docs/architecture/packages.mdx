...
iOS data collection via xctrace and devicectl. Includes:
...
- Physical device enumeration via `xcrun xctrace list devices`
- Physical device process discovery and memory collection via `xcrun devicectl` (Xcode 15+)
- Conditional collection logic based on device type (simulator vs physical)
